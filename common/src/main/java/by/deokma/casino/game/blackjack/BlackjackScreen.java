...
public class BlackjackScreen extends GameScreen<BlackjackGame, BlackjackMenu> {
    ...
    private int handLabelY()      { return bgBottom() - topPos - 80; }
        ...
        if (phase != BlackjackGame.Phase.BETTING) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (var c : hand.stream().toList()) {
                    if (c.rank() == Ranks.ACE) { rawSum += 11; aceCount++; }
                ...
                String line2; int l2col;
                if      (busted)       { line2 = chips + "♦  lost " + bet + "♦";          l2col = 0xFF7777; }
                else if (stood)        { line2 = chips + "♦  bet "  + bet + "♦  Stand";   l2col = 0xAAFFAA; }
                else if (bet > 0)      { line2 = chips + "♦  bet "  + bet + "♦";          l2col = 0xDDDDDD; }
                else                   { line2 = chips + "♦";                              l2col = 0xAAAAAA; }
    ...
    protected void renderLabels(@NotNull GuiGraphics g, int mx, int my) {
        ...
        if (phase == BlackjackGame.Phase.RESULT) {
            ...
            String resultStr; int resultColor;
            if      (bust)    { resultStr = "You bust (" + hv + ") — Dealer wins"; resultColor = 0xFF4444; }
            else if (dv > 21) { int win = bj?(int)(bet*1.5):bet; resultStr = "Dealer busts! You win +" + win + "♦"; resultColor = 0x55FF55; }
            else if (hv > dv) { int win = bj?(int)(bet*1.5):bet; resultStr = (bj ? "BLACKJACK! " : "You win! ") + "+" + win + "♦"; resultColor = 0x55FF55; }
            else if (hv == dv){ resultStr = "Push — bet returned (" + chips + "♦)"; resultColor = 0xFFFF55; }
            else               { resultStr = "Dealer wins  " + dv + " > " + hv;    resultColor = 0xFF4444; }

            int rw = font.width(resultStr);
            int ry = cardAreaCenterY();
            g.fill(cx - rw/2 - 4, ry - 1, cx + rw/2 + 4, ry + 13, 0xCC000000);
            g.drawString(font, resultStr, cx - rw / 2, ry + 1, resultColor, true);
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            String statusStr  = ""; int statusColor = 0xFFFFFF;
            if      (busted)                                                   { statusStr = "BUST!";          statusColor = 0xFF4444; }
            else if (stood)                                                    { statusStr = "Stand ✓";        statusColor = 0xAAFFAA; }
            else if (bet > 0 && phase != BlackjackGame.Phase.BETTING)         { statusStr = "Bet: " + bet; }
            else if (bet > 0)                                                  { statusStr = "✓ " + bet + "♦"; statusColor = 0xAAFFAA; }
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (phase == BlackjackGame.Phase.PLAYING && myTurn) {
            if (myIdx >= 0 && !menu.isStood(myIdx) && !menu.isBusted(myIdx)) {
                ...
                if (hit(mx, my, bx,                          by, BTN_W, BTN_H)) { sendAction(BlackjackGame.ACTION_HIT);    return true; }
                if (hit(mx, my, bx + BTN_W + BTN_GAP,       by, BTN_W, BTN_H)) { sendAction(BlackjackGame.ACTION_STAND);  return true; }
                if (canDouble && hit(mx, my, bx + 2*(BTN_W+BTN_GAP), by, BTN_W, BTN_H)) {
                    sendAction(BlackjackGame.ACTION_DOUBLE); return true;
                }
            }
        }
